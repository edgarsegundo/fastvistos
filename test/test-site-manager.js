...
async function testSiteManager() {
    try {
        ...
        console.log(
            `   Features: Blog=${currentSite.features.blog}, Booking=${currentSite.features.booking}\n`
        );
        ...
        console.log(
            `   CSS Variables: ${JSON.stringify(conceptSite.customStyles.cssVars, null, 2)}\n`
        );
        ...
        for (const test of envTests) {
            ...
            console.log(
                `${match} Domain "${test.env}" → Site ID: "${siteId}" (expected: "${test.expected}")`
            );
        }

...
describe('BlogService - Real Database Tests', () => {
  ...
  describe('getBlogConfig', () => {
    it('should fetch real blog configuration from database', async () => {
      ...
      if (result) {
        ...
        assert.strictEqual(typeof result.is_removed, 'boolean', 'is_removed should be boolean');
      ...
      if (result.length > 0) {
        ...
        result.forEach((article, index) => {
          ...
          if (article.blog_topic) {
            console.log(`      Topic: ${article.blog_topic.title} (${article.blog_topic.slug})`);
          }
          if (article.content_html) {
            console.log(`      Content Length: ${article.content_html.length} characters`);
          }
          console.log('');
        });
  ...
  describe('getRecentArticles', () => {
    it('should fetch real recent articles from database', async () => {
      ...
      console.log(`⏰ Real Recent Articles Result: Found ${result.length} recent articles (limit: 5)`);
      ...
      if (result.length > 0) {
        ...
        if (result.length > 1) {
          for (let i = 0; i < result.length - 1; i++) {
            ...
            assert.ok(current >= next, 'Articles should be ordered by published date (newest first)');
          }
        }
      } else {
  ...
  describe('getTopicsWithArticles', () => {
    it('should fetch real topics with their articles from database', async () => {
      ...
      console.log(`🗂️ Real Topics with Articles Result: Found ${result.length} topics with articles`);
      ...
      if (result.length > 0) {
        ...
        if (result.length > 0) {
          assert.ok(result[0].id, 'Topics should have IDs');
          assert.ok(result[0].title, 'Topics should have titles');
          assert.ok(Array.isArray(result[0].blog_article), 'Topics should have blog_article array');
        }
      } else {
      ...
      if (articles.length > 0) {
        ...
        if (result) {
          ...
          if (result.blog_topic) {
            console.log(`   Topic: ${result.blog_topic.title} (${result.blog_topic.slug})`);
          }
          ...
          // Verify expected structure
          assert.strictEqual(result.slug, testSlug, 'Should return article with correct slug');

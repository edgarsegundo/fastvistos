...
export class WebPageService {
    ...
    static _businessId = null;
    // Get the business_id from site config (cached)
    static getBusinessId() {
        if (this._businessId === null) {
            if (!siteConfig?.site?.business_id) {
                throw new Error('business_id not found in site configuration. Please ensure site-config.ts has a valid site.business_id.');
            }
     ...
     */
    static async createSectionAndVersion({ webpageRelativePath, title, updatableUuid }) {

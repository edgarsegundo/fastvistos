# Backlog

## 


- Colocar o titulo do artigo na págima image-uploader
- Escolher a primeira imagem da lista como default no Imagem principal do artigo?
- Preciso de algum feedback de sucesso quando clico no botão Salvar artigo
- Não está claro que eu posso clicar na imagem da lista para copiar o markdown image link
- Quando executar .l2, para os artigos publicados,  colocar link para ver como ficou e também colocar aqui:
    ✅ Indexação e publicação concluída para 'agente-imigratorio-americano-expulso-brasil-cancelamento-visto-2026-04-25' com site-id da 'centraldevistos'
      Indexing API: ✅
- external resources
- google image search
- images site links
- botao publicar no /admin/image-uploader?
- Será que eu consigo criar um agente capaz de entrar com um headless browser para fazer a parte de indexar como gsc?

